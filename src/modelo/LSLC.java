...
public class LSLC {
    ...
    public String recorre(){
        ...
        if (!esVacia()) {//Hay que tener presente que la lista no puede estar vacia
            ...
            do { //Como el fin de recorrido se acabacuando x == primero entonces primero se tiene que realizar una vez el recorrido, por lo menos
                ...
                p = p.getLiga();
                if (p!=getPrimero())mensaje+=", ";
            }while (!finDeRecorrido(p));
        }else mensaje="La lista esta vacia";
        mensaje+=".";
        return mensaje;
    }
    ...
    public void lz(LSLC lcero, LSLC ldos,LSLC ltres, LSLC lcinco){
        ...
        if (!ltres.esVacia()) {//Hay que tener presente que la lista no puede estar vacia
            p = ltres.getPrimero();
            do {
                if (p.getDato()%2!=0)insertarAlFinal(p.getDato());
                p = p.getLiga();
        ...
        if (!lcinco.esVacia()) {//Hay que tener presente que la lista no puede estar vacia
            p = lcinco.getPrimero();
            do {
                if (p.getDato()%2!=0 && p.getDato()%3!=0)insertarAlFinal(p.getDato());
                p = p.getLiga();
    ...
    public void copiar(LSLC lista){
        nodoSimple p;
        if (!lista.esVacia()) {//Hay que tener presente que la lista no puede estar vacia
            p = lista.getPrimero();
            do {
                insertarAlFinal(p.getDato());
                p = p.getLiga();
            }while (!lista.finDeRecorrido(p));
        }
    }
}
